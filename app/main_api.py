...
async def get_client_guilds(user: dict = Depends(get_current_api_user)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid user ID in Client identity")
...
async def get_client_guild_config(guild_id: int, user: dict = Depends(get_current_api_user)):
...
async def toggle_client_guild_config(guild_id: int, payload: dict, user: dict = Depends(get_current_api_user)):
    """Toggles an extension for a specific guild."""
    identity = user.get("identity", "")
    if not identity.startswith("api_key_client_"):
        raise HTTPException(status_code=403, detail="Only Client API Keys can access this endpoint")
        
    from app.ui.helpers import update_guild_extension_setting, notify_api_of_config_change
    from app.common.extension_loader import GadgetInspector
