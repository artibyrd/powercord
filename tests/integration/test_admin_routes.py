...
from app.main_ui import admin_home  # noqa: E402
...
async def test_dashboard_extensions_section(
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        ...
        # Call dashboard handler directly via await
        component = await dashboard(123, mock_session)
...
async def test_remove_admin_route(mock_remove, mock_render, mock_session):
    ...
    result = await remove_admin_route(MockReq(), mock_session)
...
async def test_render_admin_list_with_zero_id(mock_get_admins, mock_get_username, mock_session):
    ...
    component = await _render_admin_list(mock_session)
    html = to_xml(component)
